...
from typing import Any, Dict, List, Tuple, Optional, Literal, Type, Union, get_args
...
class EvoModel(LightningModule):
    def __init__(
        ...
        # Loss functions
        self.loss: _Loss = loss_functions.get_loss_fn(loss_type=loss_fn, hparams=loss_fn_hparams)
    ...
    def _step(self, batch, type_step: Literal["train", "valid", "test"], solo_step: bool = False):
        x, y = batch
        ...
        for i, block_y in enumerate(blocks_y_hat):
            ...
            self.preds = preds
            block_metrics_output = block._modules[f"{type_step}_metrics"](preds, batch[1])
    ...
    def configure_optimizers(self):
        ...
        optimizer_hparams = dict(params=self.parameters(), **self.hparams.optimizer_hparams)
        ...
        if self.hparams.lr_scheduler.lower() == "onecycle":
            optim_conf["lr_scheduler"] = dict(
                scheduler=OneCycleLR(optim_conf["optimizer"], **self.hparams.lr_scheduler_hparams), interval="step",
            )
        return optim_conf
    ...
    def _post_add_block(self, device: Optional[str]):
        ...
        for type_step in ["train", "valid", "test"]:
            block._modules[f"{type_step}_metrics"] = MetricCollection(
                deepcopy(self.hparams.metrics), prefix=f"{type_step}/", postfix=f"_block_{self.num_blocks-1}"
            ).to(device)
    ...
    def add_block_linear(
        ...
        block_weights = self._get_connection_weights(block_type)
        cxt_idx = self._select_connection_index(block_type, connection_index, block_branch=block_branch)
        block_branch = self.check_block_branch(block_branch, cxt_idx[0])
        input_type = self.get_input_type(cxt_idx)
        input_pooling = True if input_type in get_args(CONV_TYPES) else False
        input_flatten = True if input_type in get_args(CONV_TYPES) else False
        ...
        self.add_module(
            ...
            LinearBlock(
                ...
                connection_weights=block_weights,
        ...
        self.add_layer_linear(
            out_features=out_features, sparsity=sparsity, device=device,
        )

    # TODO [#LOW]: Implement groups + dilation + padding_mode
    def add_block_conv(
        self,
        # module parameters
        ...
        layers_weight_init: Optional[WEIGHT_INITS] = None,
        layers_bias_init: Optional[BIAS_INITS] = None,
        regularization: REGULARIZATIONS = "BatchNorm",
        regularization_hparams: Optional[dict] = None,
        activation_fn: ACTIVATIONS = "ReLU",
        activation_fn_hparams: Optional[dict] = None,
        # output layer parameters
        ...
        input_reshape = True if cxt_type in get_args(LINEAR_TYPES) else False
    ...
    def add_layer_linear(
        self, out_features: Optional[int], sparsity: float = 0.0, device: Optional[str] = None
    ) -> None:
        device = device or self.device
        module_type = "linear"
        ...
        dims = self._get_dims_cxt_idx(cxt_idx, block.input_pooling)
        ...
        if len(dims) == 3:
            ...
            for d in [16, 8, 4, 2]:
                if dims[0] // (d ** 2) >= 4:
                    ...
                    in_width = d
                    break
        out_channels_, kernel_size_, stride_, padding_ = block_utils.random_layer_hparams(
            in_channels, in_height, in_width, block.keep_input_dims
        )
        out_channels = out_channels or out_channels_
        kernel_size = kernel_size or kernel_size_
        stride = stride or stride_
        padding = padding or padding_
    ...
    # ** Connections
    # * Layer connections
    def add_connections_linear(self, sparsity: float, device: Optional[str] = None,) -> None:
    ...
    def remove_connections_linear(self, sparsity: float, device: Optional[str] = None) -> None:
        device = device or self.device
        self._assert_block_type("linear")
        block = self.last_block()
    ...
    # * Output connections
    def add_connections_out(self, sparsity: float, device: Optional[str] = None,) -> None:
        """
    ...
    def remove_connections_out(self, sparsity: float, device: Optional[str] = None) -> None:
        device = device or self.device
        block = self.last_block()
        block.remove_connections_out(sparsity, device)

    # ** Parameters
    def reset_parameters(self, layer_type: Optional[LAYER_TYPES] = None) -> None:
        """
        ...
        for i in range(self.num_blocks):
            block: BaseBlock = self.get_submodule(f"block_{i}")
            block.reset_parameters(layer_type)
    ...
    def _calculate_probs_layers(self, device: Optional[str] = None) -> Tensor:
        device = device or self.device
        lw = self._get_connection_weights(self.last_block().block_type)
        return lw / lw.sum()
    ...
    def _get_dims_cxt_idx(self, connection_index: Tuple[int, int], pooling: bool = False) -> Tuple[int, int, int]:
        block_idx, layer_idx = connection_index
        if block_idx == 0:
            dims = (self.hparams.data_shape[0],) if pooling else tuple(i for i in self.hparams.data_shape)
        else:
            # blocks start at index 1, layers start at index 0
            block: Union[ConvBlock, LinearBlock] = self.get_submodule(f"block_{block_idx-1}")
            ...
            if hasattr(layer, "out_channels"):
                ...
                raise ValueError(f"{layer} from block {block_idx-1} does not have out_channels or out_features")
        return dims

    def _get_connection_index(self, block_num: Optional[int] = None) -> Tuple[int, int]:
        """Get the connection index of the block"""
        if self.num_blocks == 0:
            return 0, 0
        if block_num is None:
            block = self.last_block()
            cxt_idx = block.connection_index if block.num_layers == 0 else (self.num_blocks, block.num_layers - 1)
        else:
    ...
    def _select_connection_index(
        self, block_type: BLOCK_TYPES, connection_index: Optional[tuple], counter: int = 0, block_branch: bool = False
    ) -> Tuple[int]:
        """Creates a connection to an existing layer in a block or model input.

        Eg: (0,0) refers to the model input.
            (1,0) refers to the first layer of the first block (block_0).
            (1,1) refers to the second layer of the first block.

        Args:
            connection_weights (Tensor): Importance of connecting to each block. A value of 0
            represents no connection possible.
        """
        ...
        if connection_index is None:
            ...
            if block_idx == 0:
                ...
                if block.connected_to_merge_block and block_branch:
                    ...
                    is_last_layer = random.choice([True, False]) if not self.hparams.only_last_layer_cxts else True
        ...
        # make sure that we do not connect to layer with out_width == 1 and out_height == 1
        if isinstance(self.last_block(), ConvBlock) and isinstance(connected_block, ConvBlock):
            if connected_layer.out_width == 1 and connected_layer.out_height == 1:
                if custom_cxt_idx:
                    raise ValueError("Cannot connect conv layer to conv layer with out_width == 1 and out_height == 1")
                elif counter == (tries := 100):
                    raise ValueError(
                        f"""Attempted to find a valid connection {tries} times. 
                    Consider allowing the connections to be closer to the input by changing block_weights for conv blocks."""
                    )
                return self._select_connection_index(block_type, connection_index, counter + 1, block_branch)
    ...
    def get_input_type(self, cxt_idx: Tuple[int, int]) -> BLOCK_TYPES:
        if cxt_idx[0] == 0:
            if len(self.hparams.data_shape) > 1:
                ...
                return "linear"
        block: BaseBlock = self.get_submodule(f"block_{cxt_idx[0]-1}")
    ...
    def print_grad_states(self) -> Tuple[str, bool]:
